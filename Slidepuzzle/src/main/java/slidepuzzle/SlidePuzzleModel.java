...
/**
 * All the tiles are in contents 
 * while the empty tile is defined by emptyTile
 */
    private Tile[][] contents;  // All tiles.
    private Tile emptyTile; // The empty space.

    /**
     * constructor of the class, where the dimensions of the contents are given and the puzzle is reset
     */
...
/**
 * Method to get the face value on a specific tile: e.g. "1" on the tile [0][0]
 ...
 */
     ...
     * Orders puzzle
     * 
     * Goes through the puzzle's contents and assigns face values in the correct order
     * Also places the empty tile as the very last tile
     * 
     */
 ...
 * 
 * Using the parameters of the given tile, the method checks whether there are any available empty slots and if there are, moves the tile to the empty slot
 * 
 ...
 */
     ...
     * @param rdelta the amount of rows away from the given tile that the program needs to go to check for empty tile
     * @param cdelta the amount of columns away from the given tile that the program needs to go to check for empty tile
     * @return true the neighboring tile is empty and the tiles have exchanged places, false otherwise
     */
 ...
 */
     ...
     * @return true when both the row and column values are between 0 and 2, and false otherwise
     */
 ...
 *
 * @return true if all the tiles are in the "final position" (see class tile for the final position method) 
 */
